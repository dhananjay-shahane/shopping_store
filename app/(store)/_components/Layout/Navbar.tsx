      ...
      <header className="sticky top-0 bg-white/95 backdrop-blur-md z-40 border-b border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.03)] transition-all">
        <div className="container mx-auto px-4 md:px-8 h-16 md:h-24 flex items-center justify-between relative">
          ...
          <nav className="hidden md:flex flex-1 gap-6 lg:gap-8 text-xs font-medium text-gray-600 uppercase tracking-widest items-center bg-white h-full">
            <Link href="/" className={`hover:text-pink-500 transition-colors ${pathname === '/' ? 'text-pink-500' : ''}`}>Home</Link>
            <Link href="/shop" className={`hover:text-pink-500 transition-colors ${pathname === '/shop' ? 'text-pink-500' : ''}`}>Shop All</Link>
            <Link href="/contact" className={`hover:text-pink-500 transition-colors ${pathname === '/contact' ? 'text-pink-500' : ''}`}>Contact Us</Link>
            <Link href="/studio" className={`hover:text-pink-500 transition-colors ${pathname === '/studio' ? 'text-pink-500' : ''}`}>About Us</Link>
            
            <div className="relative group h-full flex items-center">
              <span className="flex items-center gap-1 hover:text-pink-500 transition-colors h-full cursor-pointer">
          ...
          <div className="flex-1 flex justify-end items-center gap-2 md:gap-6">
            ...
            <Link href="/login" className="hidden md:block hover:text-pink-500 transition-colors text-gray-700 p-2">
              ...
              {searchQuery && (
                <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                  {PRODUCTS
                    ...
                    .map(p => (
                      <div key={p.id} onClick={() => handleProductSelect(p.id)} className="flex gap-4 items-center p-3 hover:bg-gray-50 cursor-pointer rounded-lg transition-colors">
                    ...
                    {PRODUCTS.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                      <div className="text-gray-500 text-sm py-4 text-center col-span-2">No results found matching "{searchQuery}".</div>
                    )}
                </div>
              )}
            </div>
          ...
          <div className="flex-1 overflow-hidden relative">
            <div className={`absolute inset-0 flex flex-col bg-white transition-transform duration-300 ease-in-out ${submenuOpen ? '-translate-x-full' : 'translate-x-0'}`}>
              <div className="flex-1 overflow-y-auto py-2">
                <button onClick={() => handleNavigate('/')} className="w-full text-left px-6 py-5 text-gray-800 hover:bg-gray-50 text-base font-normal tracking-wide border-b border-gray-50">
                  Home
                </button>
                <button onClick={() => handleNavigate('/shop')} className="w-full text-left px-6 py-5 text-gray-800 hover:bg-gray-50 text-base font-normal tracking-wide border-b border-gray-50">
                  Shop All
                </button>
                <button onClick={() => handleNavigate('/contact')} className="w-full text-left px-6 py-5 text-gray-800 hover:bg-gray-50 text-base font-normal tracking-wide border-b border-gray-50">
                  Contact Us
                </button>
                <button onClick={() => handleNavigate('/studio')} className="w-full text-left px-6 py-5 text-gray-800 hover:bg-gray-50 text-base font-normal tracking-wide border-b border-gray-50">
                  About Us
                </button>
                <button 
              ...
              <div className="p-6 mt-auto">
                <button onClick={() => handleNavigate('/login')} className="flex items-center gap-3 text-gray-700">
                  <User size={22} strokeWidth={1.5} />
            ...
            <div className={`absolute inset-0 flex flex-col bg-white transition-transform duration-300 ease-in-out ${submenuOpen === 'collection' ? 'translate-x-0' : 'translate-x-full'}`}>
              ...
              <div className="flex-1 overflow-y-auto py-2">
                {['Bouquet', 'Phool', 'Bags', 'Pots', 'Plushies', 'blankets', 'accessories'].map(cat => (
                  <button key={cat} onClick={() => handleNavigate(`/category/${cat.toLowerCase()}`)} className="w-full text-left px-6 py-4 text-gray-700 hover:bg-gray-50 text-base font-normal">
                    {cat}
                  </button>
                ))}
              </div>
