...
import { ChevronDown, Check, Minus, Plus, Send, Heart, Truck, Shield, RotateCcw } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background pb-16">
      <div className="container mx-auto px-4 py-8">
        <div className="text-xs uppercase tracking-widest text-neutral-500 mb-8">
           ...
           <span className="cursor-pointer hover:text-primary mx-1" onClick={() => router.push(`/category/${getCategorySlug(product.category)}`)}>{product.category}</span> / 
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="w-full lg:w-2/5 lg:sticky lg:top-24 lg:self-start">
             <div className="flex items-center gap-2 mb-2">
               {product.isNew && (
                 <span className="bg-primary text-white text-[10px] font-bold uppercase px-2 py-1 rounded">New</span>
               )}
               <span className="text-neutral-400 text-xs font-medium uppercase tracking-widest">SUIDHAGA</span>
             </div>
             
             <h1 className="text-2xl md:text-3xl text-foreground font-medium mb-3">{product.name}</h1>
             
             <div className="flex items-center gap-3 mb-6">
               <p className="text-2xl text-primary font-semibold">Rs. {product.price.toLocaleString()}</p>
               {product.originalPrice && (
                 <p className="text-lg text-neutral-400 line-through">Rs. {product.originalPrice.toLocaleString()}</p>
               )}
             </div>
             ...
             <div className="space-y-6">
                {product.hasVariants && product.colors && product.colors.length > 0 && (
                  <div>
                    <label className="block text-xs font-medium uppercase tracking-widest text-neutral-700 mb-3">
                      Color: <span className="font-normal normal-case text-neutral-500">{selectedColor}</span>
                    </label>
                ...
                <div>
                   <label className="block text-xs font-medium uppercase tracking-widest text-neutral-700 mb-3">Quantity</label>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 py-2">
                     ...
                     <label className="text-sm text-neutral-600 cursor-pointer select-none" onClick={() => setAddGiftWrap(!addGiftWrap)}>
                ...
                <div className="bg-accent/50 rounded-lg p-4 text-sm text-neutral-600">
                  <p className="font-medium text-foreground mb-1">Handcrafted with Care</p>
                  <p>Each piece is made to order. Please allow 5-7 days for preparation before shipping.</p>
                </div>
                ...
                <div className="flex flex-col gap-3 pt-2">
                   ...
                   >
                     Add to cart - Rs. {((product.price * quantity) + (addGiftWrap ? 50 : 0)).toLocaleString()}
                   </button>
                   <button 
                     onClick={() => { handleAddToCart(); setCartOpen(true); router.push('/checkout'); }}
             ...
             <div className="mt-8 pt-6 border-t border-neutral-100">
               ...
               <div className="text-sm text-neutral-600 leading-relaxed">
                 {activeTab === 'description' && (
                   <p>{product.description} This piece is handcrafted with premium quality yarn and materials. Perfect for gifting or adding a unique touch to your home decor.</p>
                 )}
                 {activeTab === 'details' && (
                   <ul className="space-y-2">
                     <li><span className="font-medium text-foreground">Material:</span> Premium quality acrylic yarn</li>
                     <li><span className="font-medium text-foreground">Craft:</span> Handmade crochet</li>
                     <li><span className="font-medium text-foreground">Made in:</span> India</li>
                     <li><span className="font-medium text-foreground">Customization:</span> Available on request</li>
                   </ul>
                 )}
                 {activeTab === 'care' && (
                   <ul className="space-y-2">
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-24 pt-12 border-t border-neutral-100">
             <h3 className="text-2xl text-foreground mb-8 font-medium">You may also like</h3>
